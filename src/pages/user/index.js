...
export class UnconnectedUser extends Component {
  ...
  render() {
    ...
    const userName = this.props.match.params.URL_Param
      ? this.props.match.params.URL_Param
      : staticContext.URL_Param;
    /* eslint-enable */
              ...
              <div className="grid userProfilePage" data-test="userComponent">
                ...
                <div className="column column_4_12">
                  <div className="profileImage">
                    ...
                    {data.profilePage.profileImage && (
                      <img
                        src={`${webConfig.siteURL}/user-uploads/${data.profilePage.profileImage}`}
                      />
                    )}
                  </div>
                  <div className="user">{data.profilePage.businessName}</div>
                </div>
                <div className="column column_8_12">
                  <h2>About me:</h2>
                  <p>
                    {data.profilePage.street}
                    <br />
                    {data.profilePage.townCity}
                    <br />
                    {data.profilePage.postCode}
                  </p>
                  <div
                    dangerouslySetInnerHTML={{ __html: data.profilePage.bio }}
                  ></div>
                </div>
              </div>
            );
